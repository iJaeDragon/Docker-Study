...
컨테이너 오케스트레이션은 여러 PC(노드)에 컨테이너를 분산 배치하고, 특정 PC나 컨테이너에 장애가 발생하더라도 다른 노드에서 서비스를 계속 제공할 수 있도록 하며, 배포·확장·장애 복구 등을 자동으로 관리하기 위해 사용한다.
